...
*/
  ...
  editor.on('init', function (e) {
    let convertToDOM = $.parseHTML(editor.getContent());
    console.log("HTML OBJ ", convertToDOM);

    let i = 1;

    Object.keys(convertToDOM).forEach(function (key) {
      //console.log("MY LOOP: ", key, convertToDOM[key]);
      if (convertToDOM[key].tagName == 'P') {
        //console.log("Tag: ", key, convertToDOM[key].tagName);
        console.log("Tag: ", key, convertToDOM[key]);
        console.log("ClassList: ", key, $(convertToDOM[key]).attr('class'));
        if ($(convertToDOM[key]).attr('class')) {
          if ($(convertToDOM[key]).attr('class') == 'related_content') {
            relatedContPos = i;
          }
          if ($(convertToDOM[key]).attr('class') == 'video_content') {
            relatedVidPos = i;
          }
        }
      ...
      i++;
      console.log("Related Cont POS: " + relatedContPos);
      console.log("Related Vid POS: " + relatedVidPos);
    });
...
/**/
  editor.on('keydown', function (e) {
    ...
    console.log("Full Content", editor.getContent());
    ...
    onclick: function() {
      ...
      editor.windowManager.open({
        ...
        onsubmit: function(e) {
          ...
          console.log(bookMarkLoc);
          tinymce.activeEditor.selection.moveToBookmark(bookMarkLoc);
          console.log("WIDGET LOC:", e.data.enterwidget );
          // Insert content when the window form is submitted
          editor.insertContent(e.data.enterwidget);
        }
      });
